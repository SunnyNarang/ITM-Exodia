...
<span style="padding:0;font-size:15px;font-weight:800;text-align: left;">Select Sem</span>
				<select name="statnotes_sem" id="notes_sem" style="width: 100%;font-family: 'Titillium Web';font-size:15px;font-weight: 200;padding: 2px;margin-top: 2px;color: #555;border-color: #ccc;border-radius: 3px;">
...
$(document).ready(function() {				
	$('select[name=statnotes_sem]').change(function() {
		var sem = $(this).val();
		var course = $('select[name=statnotes_course]').val();
		var branch = $('select[name=statnotes_branch]').val();
		statgetnoclasss(branch, course, sem);
    });
});
</script>				