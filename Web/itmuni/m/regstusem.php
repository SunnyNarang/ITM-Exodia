...
<span style="padding:0;font-size:15px;font-weight:800;text-align: left;">Select Sem</span>
				<select name="regstu_sem" id="notes_sem" style="width: 100%;font-family: 'Titillium Web';font-size:15px;font-weight: 200;padding: 2px;margin-top: 2px;color: #555;border-color: #ccc;border-radius: 3px;">
				<option value="">Select a Sem</option>
<?php
echo $postedbranch;
$stmt = $conn->prepare('SELECT distinct sem from temp_students where course=? and branch= ? order by sem');
...
$(document).ready(function() {				
	$('select[name=regstu_sem]').change(function() {
		var sem = $(this).val();
		var course = $('select[name=regstu_course]').val();
		var branch = $('select[name=regstu_branch]').val();
		regstunoclasss(branch, course, sem);
    });
});
</script>				