...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
	   ...
	   $class_id=$row['id'];
	   ?>
	   <a class="itsclass" id="<?php echo $class_id; ?>"><?php echo $name; ?></a>
    <?php }
}?>
</div><div id="helloflist" style="padding:0;margin:0;width:100%"></div>
  <script type="text/javascript"> 
      $(document).ready(function(){
		  
		  
	  $('.itsclass').click(function() {
	var alt = $(this).attr('id');
	$('.itsclass').removeClass('newitsclass');
	$(this).addClass('newitsclass');
	getlist(alt);
	});
	  });
		  </script>
