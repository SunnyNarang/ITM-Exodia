...
$parent=$_GET['parent'];
include('db.php');
$stmt = $conn->prepare('SELECT notice.id as id, notice.title as title, notice.body as body, notice.notice_date as n_date, teacher.id as tea_id, teacher.name as t_name FROM notice inner join teacher on teacher.id=notice.t_id where notice.notice_date = ? Order by notice.notice_date');
