...
$stmt1 = $conn->prepare("update `attendance_conf_faculty` set `conf_fname`=? , stu_reg_id=? ,user=? where `conf_subcode`=?  and 
`conf_taken_dates` = '".date("Y-m-d")."' and `conf_tid` = ? and period = ? and periodTo = ? and att_date=? and timeToFr=? and sectionid=?");
$stmt1->bind_param('ssssssssss',$teacher1,$attendance,$ccid,$subject,$tid,$period,$periodto,$date,$time,$section);
$stmt1->execute();
